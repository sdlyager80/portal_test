...
export default function BillingManagement({ service }) {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError(null);
      const [billingRes, servicesRes] = await Promise.all([
        service.getBillingRecords(),
        service.getServiceDefinitions('billing'),
      ]);
      setBillingRecords(billingRes.result ?? []);
  ...
  return (
    <Box>
      <TaskTable
        ...
        createLabel="New Billing Task"
        searchPlaceholder="Search billing tasks…"
        emptyMessage="No billing tasks found"
      />

      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Create Billing Task</DialogTitle>
        <DialogContent>
